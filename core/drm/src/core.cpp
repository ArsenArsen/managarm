...
async::result<void>
drm_core::File::ioctl(void *object, managarm::fs::CntRequest req, helix::UniqueLane conversation) {
	auto self = static_cast<drm_core::File *>(object);
	if (req.command() == DRM_IOCTL_VERSION) {
		...
		if (req.drm_capability() == DRM_CAP_TIMESTAMP_MONOTONIC) {
			...
			std::cout << "core/drm: Unknown capability " << req.drm_capability()
			          << std::endl;
		...
		if (!resp.drm_obj_property_ids_size()) {
			std::cout << "\e[31mcore/drm: No properties found for object id "
