...
struct DmalogDevice final
	...
	DmalogDevice(
	        ...
	        : IrqSink { frg::string<KernelAlloc> { *kernelAlloc, "dmalog-" } + tag
	...
	coroutine<frg::expected<Error>> issueIo(IoFlags flags) override {
		...
		if (!outPending_ && (flags & ioProgressOutput)) {
			...
			*outDesc_ = { .flags = 1 };
			...
			while (progress < size) {
				...
				outDesc_->buffers[k] = { .ptr = outPhysical_ + misalign,
					                 .length = chunk };
				progress += chunk;
				++k;
			}
		...
		if (!inPending_ && (flags & ioProgressInput)) {
			...
			*inDesc_ = { .flags = 1 };
			...
			while (progress < size) {
				...
				inDesc_->buffers[k] = { .ptr = inPhysical_ + misalign,
					                .length = chunk };
				progress += chunk;
				++k;
			}
		...
		if (inIrq) {
			...
			updateReadableSpan({ inView_ + (inTail_ & (ringSize - 1)),
			                     inHead_ - inTail_ });
			inPending_ = false;
		}
