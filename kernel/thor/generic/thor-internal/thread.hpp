...
requires(!std::is_same_v<H, void>) Ptr<T, void> remove_tag_cast(const Ptr<T, H> &other) {
...
public:
	...
	create(smarter::shared_ptr<Universe> universe,
		...
		ptr->_pagingWorkQueue.selfPtr = remove_tag_cast(
		        smarter::shared_ptr<WorkQueue, ActiveHandle> { sptr,
	...
	requires(!std::is_same_v<typename Sender::value_type, void>) static
	        typename Sender::value_type asyncBlockCurrent(Sender s, WorkQueue *wq) {
		auto thisThread = getCurrentThread();
