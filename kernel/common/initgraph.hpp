...
struct Requires {
	...
	requires(std::is_convertible_v<Args, Node *> &&...) Requires(Args &&...args)
	        : array { { args... } } {}
...
struct Entails {
	template<typename... Args>
	Entails(Args &&...args) requires(std::is_convertible_v<Args, Node *> &&...)
	        : array { { args... } } {}
...
struct Task final : Node {
	Task(Engine *engine, const char *displayName, Requires<NR> r, Entails<NE> e, F invocable)
	        ...
	        , eEdges_ {
		        apply(std::make_index_sequence<NE> {}, e.array, IntoEdgesFrom { this })
	        } {}
