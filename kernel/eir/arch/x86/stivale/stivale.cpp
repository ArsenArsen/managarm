...
namespace TagIds {
constexpr static uint64_t cmdline = 0xe5e76a1b4597a781;
constexpr static uint64_t memoryMap = 0x2187f79e8612de07;
constexpr static uint64_t framebuffer = 0x506461d2950408fa;
constexpr static uint64_t modules = 0x4b6fe466aade04ce;
constexpr static uint64_t rsdp = 0x9e1786930a375e78;
constexpr static uint64_t dtb = 0xabb29bd49a2833fa;
}  // namespace TagIds
...
constexpr static const char *mmapEntryString(MmapEntryType t) {
	switch (t) {
	case MmapEntryType::usable:
		return "Usable";
	...
	case MmapEntryType::acpiNvs:
		return "ACPI NVS";
	case MmapEntryType::badMemory:
		return "Bad memory";
	case MmapEntryType::bootloaderReclaimable:
		return "Bootloader reclaimable";
	case MmapEntryType::kernelAndModules:
...
extern "C" void eirStivaleMain(Stivale2Struct *data) {
	...
	while (ptr) {
		...
		case TagIds::framebuffer: {
			...
			setFbInfo(reinterpret_cast<void *>(t.addr), t.width, t.height, t.pitch);
			break;
		}
		...
		case TagIds::modules: {
			...
			modules = {
				reinterpret_cast<Stivale2Module *>(
	...
	eir::infoLogger() << "Booted by: \"" << data->brand << "\", version: \"" << data->version
			  << "\"" << frg::endlog;
